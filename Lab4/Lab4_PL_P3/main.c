...
/****************************************************************************************/ 
/***** Enunciado: Se genera una señal mediante la activación por el SW2 que se puede invertir. *****/ 
/***** *****/ 
/****************************************************************************************/
...
/* Función que configura el pin PB2 y PB3 como salida para generar la señal cuadrada
Entradas: Ninguna
Salidas: Ninguna
*/
void config_portB (void) {
	 ...
	 GPIO_PORTB_DIR_R |= 0x0C; // Configura PB2 y PB3 como salida
	 GPIO_PORTB_AFSEL_R &= ~(0x0C); // No utilizar funciones alternas
	 GPIO_PORTB_DR8R_R |= 0x0C; // Activa el driver de 8 mA en PB2 y PB3
	 GPIO_PORTB_DEN_R |= 0x0C; // Habilita la señal digital en PB2 y PB3
	 GPIO_PORTB_DATA_R &=~(0x0C); // Señal en "0 lógico"
}// fin config_portB
...
Salidas: Ninguna
*/
void genera_tren ( uint8_t t_alta, uint8_t ciclos, uint32_t periodo, uint8_t inversion){
	uint8_t a; // Variable para contar parpadeos 
	uint32_t i; // Variable para retardos
	uint32_t cont; //Variable contar tiempo de señal
	
	if ((valida_tiempo ( t_alta)) && (valida_periodo (periodo))){
		if (inversion%2==0){
			for(a=0; a<ciclos; a++){ //Ciclo for para repetir la señal "ciclos" veces
				GPIO_PORTB_DATA_R |= 0x04;	//PB2 se pone en alta (1 logico")
				GPIO_PORTB_DATA_R |= 0x08;	//PB3 se pone en alta (1 logico")
				for(cont=0;cont<5*periodo/100;cont++){}	//Se mantiene en alta PB3 y alto PB2 5% del "periodo"
				GPIO_PORTB_DATA_R &=~(0x08); //PB3 se pone en baja ("0 lógico*")
				for(cont=0;cont<((periodo*t_alta/100)-(5*periodo/100));cont++){}	//Se mantiene en alta "t_alta" del "periodo" menos el 5% del periodo que ya transcurrio
			...
			for(a=0; a<ciclos; a++){ //Ciclo for para repetir la señal "ciclos" veces
				GPIO_PORTB_DATA_R &=~(0x04); //PBS se pone en baja ("0 lógico*")
				GPIO_PORTB_DATA_R |= 0x08;	//PB3 se pone en alta (1 logico")
				for(cont=0;cont<5*periodo/100;cont++){}	//Se mantiene en alta PB3 y bajo PB2 5% del "periodo"
				GPIO_PORTB_DATA_R &=~(0x08); //PB3 se pone en baja ("0 lógico*")
				for(cont=0;cont<((periodo-periodo*t_alta/100)-(5*periodo/100));cont++){}  //Se mantiene en alta "t_alta" del "periodo" menos el 5% del periodo que ya transcurrio
				GPIO_PORTB_DATA_R |= 0x04;	//PB2 se pone en alta (1 logico")
...
int main (void){
	...
	t_alta=80; 			//Valor multiplo de 10 entre 10 a 100
	periodo=periodo_min*4;	//Multiplo de 20000, menor a 2000000
	...
	//Ciclo
	while(1){
		for(a=0; a<2; a++){ //Ciclo para registro de las 2 variables
			while((GPIO_PORTF_DATA_R & 0x11) != 0x01);  //Se espera se presione SW1
			//Se registra en valor de ciclos a partir de la funcion aleatoria
			ciclos=aleatorio(); 		
			//Confirmación Ingreso primer numero aleatorio
			GPIO_PORTF_DATA_R |= ledAzul;  //Enciende led cian
			for(i = 0; i < 800000; i++); 	//Retraso
			GPIO_PORTF_DATA_R &= ~leds; 	//Apaga leds
			for(i = 0; i < 800000; i++);	//Retraso
			while((GPIO_PORTF_DATA_R & 0x11) != 0x01);  //Se espera se presione SW1
			//Se registra el valor del número aleatorio
			inversion=aleatorio();  //Se registra en valor de inversión a partir de la funcion aleatoria
			//Confirmación ingreso del segundo Número aleatorio
			GPIO_PORTF_DATA_R |=ledVerde;  //Enciende led cian
			for(i = 0; i < 800000; i++); 	//Retraso
			GPIO_PORTF_DATA_R &= ~leds; 	//Apaga leds
			for(i = 0; i < 800000; i++);	//Retraso
		}
		while((GPIO_PORTF_DATA_R & SW2) != 0);  //Se espera se presione SW2
		if ((GPIO_PORTF_DATA_R & SW2) == 0){ //Se valida que el SW2 fue presionado
			while ((GPIO_PORTF_DATA_R & SW2) != SW2);		//Se espere se suelte el SW2
			genera_tren (t_alta, ciclos, periodo, inversion);   	//Funcion que genera el tren de señales
			//Confirmación ingreso del segundo Número aleatorio
			GPIO_PORTF_DATA_R |=ledRojo;  //Enciende led cian
			for(i = 0; i < 800000; i++); 	//Retraso
			GPIO_PORTF_DATA_R &= ~leds; 	//Apaga leds
			for(i = 0; i < 800000; i++);	//Retraso
		}
